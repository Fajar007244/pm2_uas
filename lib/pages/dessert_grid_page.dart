...
class DessertGridPage extends StatefulWidget {
  const DessertGridPage({super.key});

  @override
  _DessertGridPageState createState() => _DessertGridPageState();
}

class _DessertGridPageState extends State<DessertGridPage> {
  final Logger _logger = Logger('DessertGridPage');
  List<Map<String, dynamic>> _dessertItems = [];
  ...
  Future<void> _fetchDessertItems() async {
    try {
      ...
      setState(() {
        _dessertItems = responseList.map((item) {
      ...
      _logger.e('Error fetching dessert items', error: e);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal memuat dessert: ${e.toString()}')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return _isLoading
        ...
        : _dessertItems.isEmpty
            ? const Center(
                child: Text(
                  'Tidak ada dessert tersedia',
            ...
            : GridView.builder(
                ...
                itemBuilder: (context, index) {
                  final product = _dessertItems[index];
